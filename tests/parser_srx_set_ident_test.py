...
parser_srx = ParserSrxSets(dummy_conf_data)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            'set groups COMMON security policies from-zone <*>' # Group 1
            ' to-zone <*> policy DEFAULT-POLICY then deny',
            False
        ),
        (
            'set groups COMMON-POL security policies from-zone' # Group 2
            ' TRUST to-zone UNTRUST policy ALLOW-DNS match source-address any',
            False
        ),
    ]
)
def test_ident_comm_fw_rules(command, result):
    tested = parser_srx._ParserSrxSets__identify_comm
    if result:
        assert tested(command) == 'fw rules'
    else:
        assert tested(command) == ''
